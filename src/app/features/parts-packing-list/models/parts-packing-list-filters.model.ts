 ...
 * Parameters (Invoice Number/Delivery Number) and Filter Menu (Date Range) sections. Only
 * `dateFrom`/`dateTo` are sent to the real data API (as `fromDate`/`toDate`, alongside the
 * invisible `dealerCode`) — Invoice/Delivery Number are applied as client-side filters over
